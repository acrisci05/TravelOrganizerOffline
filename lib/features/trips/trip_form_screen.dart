...
// Form di inserimento/modifica di un viaggio. Se [existingTrip] è valorizzato
// la schermata è in modalità modifica, altrimenti crea un nuovo viaggio.
...
class _TripFormScreenState extends State<TripFormScreen> {
        ...
        child: ListView(
          ...
          children: [
            ...
            Row(
              children: [
                const Icon(Icons.sell_outlined,
                    size: 18, color: Colors.grey),
                const SizedBox(width: 8),
                Text('Tag del viaggio',
                    style: Theme.of(context).textTheme.titleSmall),
              ],
            ),
            const SizedBox(height: 4),
            const Text(
              'Servono a generare automaticamente la lista valigia',
            ...
            Wrap(
              spacing: 8,
              runSpacing: 4,
              children: PackingCatalog.tags.map((tag) {
  ...
  Future<void> _save() async {
    ...
    // Integrità dei dati: impedisce di avere due viaggi con date sovrapposte.
    final overlap = provider.overlappingTrip(
      _startDate,
      _endDate,
      excludeId: widget.existingTrip?.id,
    );
    if (overlap != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Le date si sovrappongono al viaggio "${overlap.title}" '
              '(${DateFormatter.dateRange(overlap.startDate, overlap.endDate)}). '
