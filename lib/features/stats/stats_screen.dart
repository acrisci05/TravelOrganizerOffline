...
// Schermata "Statistiche": riepiloghi e analisi su tutti i viaggi (totali, stato,
// attività, spese, checklist, classifica tappe e salute del budget per viaggio).
...
// Barra "salute del budget": cambia colore in modo progressivo (verde sotto al
// 75%, giallo tra il 75% e il 100%, rosso oltre il budget) per dare un feedback
// visivo immediato sull'andamento delle spese.
class _BudgetBar extends StatelessWidget {
  final double spent;
  final double budget;
  const _BudgetBar({required this.spent, required this.budget});

  // Restituisce il colore in base alla percentuale di budget utilizzata.
  static Color healthColor(double pct) {
  ...
  Widget build(BuildContext context) {
    ...
    final ratio = pct.clamp(0.0, 1.0);
    final overBudget = spent > budget;
    final color = healthColor(pct);
    return Column(
        ...
        Text(
          overBudget
              ...
              : 'Disponibile: ${DateFormatter.currency(budget - spent)} '
                  '(${(pct * 100).toStringAsFixed(0)}% usato)',
